  ...
  return (
    <section className="operation-execution" aria-label="Operation execution metrics">
      ...
      <div className="operation-execution__tables">
        ...
        <div className="operation-execution__table">
          <table><caption>Action metrics</caption><thead><tr><th>Action</th><th>Kind</th><th>Completed</th><th>Failed</th><th>Avg.</th><th>Records</th><th>Bytes</th><th>Why</th></tr></thead><tbody>{actions.map((action) => { const searchDetails = action.details?.searchFanOut === undefined ? '' : ` · fan-out ${action.details.searchFanOut} · candidates ${action.details.searchCandidates ?? 0} · results ${action.details.searchResultCount ?? 0}${action.details.searchStale ? ` · stale ${action.details.searchVisibilityLagMs ?? 0} ms` : ''}`; const realtimeDetails = action.details?.realtimeFanOut === undefined ? '' : ` · ${action.details.realtimeFanOut} connection fan-out${action.details.realtimeChannelId ? ` · ${action.details.realtimeChannelId}` : ''}`; return <tr key={`${action.operationId}:${action.actionId}`}><td><code>{action.actionId}</code></td><td>{action.actionKind}</td><td>{action.completed.toLocaleString()}</td><td>{action.failed.toLocaleString()}</td><td>{action.averageDurationMs.toLocaleString()} ms</td><td>{action.recordsExamined.toLocaleString()}</td><td>{action.bytesProcessed.toLocaleString()}</td><td>{action.explanation ?? '—'}{searchDetails}{realtimeDetails}</td></tr> })}</tbody></table>
        </div>
      </div>
    </section>
  )
