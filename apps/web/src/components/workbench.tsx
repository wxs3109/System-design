...
import { createAsyncExample, createCollaborativeEditingExample, createDataPlatformExample, createDirectExample, createIncidentFanOutExample, createLogSearchExample, createOrderEventFanOutExample, createOrderFulfillmentWorkflowExample, createOrderSystemExample, createPaymentCheckoutWorkflowExample, createProductSearchExample, createRealtimeChatExample, createScheduledBatchExample, createVideoDeliveryExample } from '@/lib/examples'
...
function formatDomainMetrics(details: SimulationResult['nodes'][number]['details']) {
  const preferred = Object.keys(domainMetricLabels).flatMap((key) => key in details ? [[key, details[key]] as const] : []).slice(0, 3)
  if (preferred.length === 0) return '—'
  return preferred.map(([key, value]) => {
    const formatted = typeof value === 'number' && ['cacheHitRate', 'cdnHitRate', 'cacheOccupancy', 'partitionImbalance', 'popRequestImbalance', 'hottestShardShare', 'searchStaleQueryRate'].includes(key)
