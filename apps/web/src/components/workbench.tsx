...
import { createAsyncExample, createDataPlatformExample, createDirectExample, createOrderSystemExample } from '@/lib/examples'
  ...
  return (
    <section className="operation-execution" aria-label="Operation execution metrics">
      ...
      <div className="operation-execution__tables">
        ...
        <div className="operation-execution__table">
          <table><caption>Action metrics</caption><thead><tr><th>Action</th><th>Kind</th><th>Completed</th><th>Failed</th><th>Avg.</th><th>Records</th><th>Bytes</th><th>Why</th></tr></thead><tbody>{actions.map((action) => <tr key={`${action.operationId}:${action.actionId}`}><td><code>{action.actionId}</code></td><td>{action.actionKind}</td><td>{action.completed.toLocaleString()}</td><td>{action.failed.toLocaleString()}</td><td>{action.averageDurationMs.toLocaleString()} ms</td><td>{action.recordsExamined.toLocaleString()}</td><td>{action.bytesProcessed.toLocaleString()}</td><td>{action.explanation ?? '—'}</td></tr>)}</tbody></table>
        </div>
      </div>
    </section>
  )
