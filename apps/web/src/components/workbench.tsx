...
import { createAsyncExample, createCollaborativeEditingExample, createDataPlatformExample, createDirectExample, createGlobalStorefrontExample, createIncidentFanOutExample, createLogSearchExample, createMultiRegionFailoverExample, createOrderEventFanOutExample, createOrderFulfillmentWorkflowExample, createOrderSystemExample, createPaymentCheckoutWorkflowExample, createProductSearchExample, createRealtimeChatExample, createScheduledBatchExample, createVideoDeliveryExample } from '@/lib/examples'
...
function RegionSection() {
  ...
  return (
    <section className="region-section" aria-label="Regions and zones">
      <div className="policy-section__heading"><span>Regions / zones</span><small>Define routing locality and outage boundaries</small></div>
  ...
  return (
    <section className="operation-execution" aria-label="Operation execution metrics">
      ...
      <div className="operation-execution__tables">
        ...
        <div className="operation-execution__table">
          <table><caption>Action metrics</caption><thead><tr><th>Action</th><th>Kind</th><th>Completed</th><th>Failed</th><th>Avg.</th><th>Records</th><th>Bytes</th><th>Why</th></tr></thead><tbody>{actions.map((action) => { const searchDetails = action.details?.searchFanOut === undefined ? '' : ` · fan-out ${action.details.searchFanOut} · candidates ${action.details.searchCandidates ?? 0} · results ${action.details.searchResultCount ?? 0}${action.details.searchStale ? ` · stale ${action.details.searchVisibilityLagMs ?? 0} ms` : ''}`; const realtimeDetails = action.details?.realtimeFanOut === undefined ? '' : ` · ${action.details.realtimeFanOut} connection fan-out${action.details.realtimeChannelId ? ` · ${action.details.realtimeChannelId}` : ''}`; const workflowDetails = action.details?.workflowStatus === undefined ? '' : ` · workflow ${action.details.workflowStatus}${action.details.workflowIdempotencyReplay ? ' · idempotency replay' : ''}`; return <tr key={`${action.operationId}:${action.actionId}`}><td><code>{action.actionId}</code></td><td>{action.actionKind}</td><td>{action.completed.toLocaleString()}</td><td>{action.failed.toLocaleString()}</td><td>{action.averageDurationMs.toLocaleString()} ms</td><td>{action.recordsExamined.toLocaleString()}</td><td>{action.bytesProcessed.toLocaleString()}</td><td>{action.explanation ?? '—'}{searchDetails}{realtimeDetails}{workflowDetails}</td></tr> })}</tbody></table>
        </div>
      </div>
    </section>
  )
  ...
  const canvasPanel = (
      <section className="canvas-stage" onDrop={onDrop} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move' }}>
        ...
        >
          ...
          <Panel position="top-left"><div className="canvas-toolbar"><button type="button" onClick={() => { setProject(createEmptyProject()); reactFlow.setCenter(0, 0, { zoom: 1 }) }}><RotateCcw size={14} /> Clear canvas</button><div className="example-picker"><button type="button" aria-expanded={exampleOpen} onClick={() => setExampleOpen((open) => !open)}><Save size={14} /> Load example <ChevronDown size={13} /></button>{exampleOpen ? <div className="example-menu"><button type="button" onClick={() => { setProject(createOrderSystemExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Order system</strong><span>APIs → Cache → Relational data → Events</span></button><button type="button" onClick={() => { setProject(createScheduledBatchExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Scheduled batch</strong><span>Scheduler → Queue → Workers → Database</span></button><button type="button" onClick={() => { setProject(createVideoDeliveryExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Video delivery</strong><span>Viewers → CDN POPs → Edge cache / Origin</span></button><button type="button" onClick={() => { setProject(createProductSearchExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Product search</strong><span>Queries + catalog updates → sharded Search Index</span></button><button type="button" onClick={() => { setProject(createLogSearchExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Log search</strong><span>Streaming ingest + investigations → Search Index</span></button><button type="button" onClick={() => { setProject(createOrderEventFanOutExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Order event fan-out</strong><span>Orders → Topic → independent fulfillment / email subscriptions</span></button><button type="button" onClick={() => { setProject(createIncidentFanOutExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Incident fan-out</strong><span>Alerts → Topic → independent subscribers + retention expiry</span></button><button type="button" onClick={() => { setProject(createRealtimeChatExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Realtime chat</strong><span>Long-lived clients → rooms → broadcast + slow-client backpressure</span></button><button type="button" onClick={() => { setProject(createCollaborativeEditingExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Collaborative editing</strong><span>Editors → document channels → operation broadcast + disconnect</span></button><button type="button" onClick={() => { setProject(createPaymentCheckoutWorkflowExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Payment checkout</strong><span>Checkout → durable steps → idempotent completion</span></button><button type="button" onClick={() => { setProject(createOrderFulfillmentWorkflowExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Order fulfillment</strong><span>Allocation → retry exhaustion → reverse compensation</span></button><button type="button" onClick={() => { setProject(createGlobalStorefrontExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Global storefront</strong><span>Regional shoppers → geo routing → regional catalogs</span></button><button type="button" onClick={() => { setProject(createMultiRegionFailoverExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Multi-region failover</strong><span>Cached primary route → outage → delayed standby failover</span></button><button type="button" onClick={() => { setProject(createDirectExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Direct service</strong><span>Traffic → Network → Service → DB</span></button><button type="button" onClick={() => { setProject(createAsyncExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Async pipeline</strong><span>Traffic → API → Queue → Worker → DB</span></button><button type="button" onClick={() => { setProject(createDataPlatformExample()); setExampleOpen(false); setTimeout(() => reactFlow.fitView(), 0) }}><strong>Data platform</strong><span>Cache → Shards → Stream → Objects</span></button></div> : null}</div></div></Panel>
        </ReactFlow>
