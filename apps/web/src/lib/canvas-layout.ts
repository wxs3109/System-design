...
export async function layoutTopology(project: ProjectFile, dimensions?: CanvasNodeDimensionsMap): Promise<Record<string, { x: number; y: number }>> {
  ...
  const graph: ElkNode = {
    ...
    children: project.topology.nodes.map((node) => ({ id: node.id, ...dimensionsForNode(node.id, dimensions) })),
