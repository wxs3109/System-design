...
export function calculateTopologyGroupBounds(project: ProjectFile): TopologyGroupBounds[] {
  ...
  return project.topology.groups.flatMap((group) => {
    ...
    const maxX = Math.max(...members.map((node) => node.position.x + canvasNodeSize.width))
    const maxY = Math.max(...members.map((node) => node.position.y + canvasNodeSize.height))
