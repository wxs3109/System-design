...
test('loads the end-to-end video platform and exposes upload, transcode, metadata and streaming paths', async ({ page }) => {
  await page.goto('/')
  await openExamplePicker(page)
  await page.getByRole('button', { name: /Video delivery/ }).click()
  await expect(page.getByText('14 components')).toBeVisible()
  await expect(page.getByTestId('rf__node-video-upload-streams')).toContainText('Multipart uploader')
  await expect(page.getByTestId('rf__node-video-upload-api')).toContainText('Video upload API')
  await expect(page.getByTestId('rf__node-transcode-queue')).toContainText('Transcode queue')
  await expect(page.getByTestId('rf__node-transcoder-workers')).toContainText('Transcoder workers')
  await expect(page.getByTestId('rf__node-video-metadata-db')).toContainText('Video metadata DB')
  await expect(page.getByTestId('rf__node-video-rendition-storage')).toContainText('Encoded renditions')
  await page.getByTestId('rf__node-video-cdn').dispatchEvent('click')
  await expect(page.getByLabel('POP count')).toHaveValue('12')
  await expect(page.getByLabel('POP selection')).toHaveValue('consistent-hash')
  await expect(page.getByLabel('Edge bandwidth (Mbps)')).toHaveValue('10000')
  await expect(page.getByLabel('Origin round trip (ms)')).toHaveValue('75')

  await page.getByRole('button', { name: 'Definitions' }).click()
  const definitions = page.getByRole('navigation', { name: 'Project definitions' })
  await expect(definitions).toContainText('Video upload API')
  await expect(definitions).toContainText('Playback API')
  await expect(definitions).toContainText('Video metadata')
  await expect(definitions).toContainText('Transcode requested')

  await page.getByRole('button', { name: 'Run simulation' }).click()
  await expect(page.getByText('Throughput over virtual time')).toBeVisible({ timeout: 15_000 })
  const row = page.getByRole('row').filter({ hasText: 'Video CDN' })
...
test('labels connections and focuses the exact path for a selected interaction', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Definitions' }).click()
  const explorer = page.getByRole('navigation', { name: 'Project definitions' })
  await explorer.getByRole('button', { name: /Get order with cache aside/ }).click()

  await expect(page.getByTestId('rf__edge-client-to-orders')).toHaveClass(/is-definition-binding/)
  await expect(page.getByTestId('rf__edge-orders-to-cache')).toHaveClass(/is-definition-binding/)
