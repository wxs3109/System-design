...
Implement P2.6e Realtime Gateway next and commit it as an independent behavior settlement. It must model long-lived connection capacity, channel/room membership, broadcast amplification, and connection backpressure with result-changing tests; do not freeze a public SDK until these later behavior variants have exposed the extension points it actually needs.
