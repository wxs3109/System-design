...
Implement P2.6g Global Router next and commit it as an independent behavior settlement. It must model geo/weighted/health routing, cached decisions, TTL, and observable failover delay with result-changing tests; do not freeze a public SDK until this final planned behavior variant has exposed the extension points it actually needs.
