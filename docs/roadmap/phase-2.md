...
Implement P2.6f Workflow next and commit it as an independent behavior settlement. It must model durable step state, idempotency, timeout, bounded retry, and compensation with result-changing tests; do not freeze a public SDK until these later behavior variants have exposed the extension points it actually needs.
