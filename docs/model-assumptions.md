...
- A request reaching a Global Router uses the Region of its workload source as its client location and the Region of each direct route target as target location. Layout coordinates are ignored. The decision is cached under its workload cohort identity; another root source without that identity falls back to its sampled key or trace ID. Operation action paths invoke the same geo, weighted, health-aware, cache, and failover selection state when their compiled path traverses a Global Router.
