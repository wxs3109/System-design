...
describe('runtime component behavior registry', () => {
  it('provides behavior for every built-in component', () => {
    expect(registeredBehaviorTypes()).toEqual(['traffic', 'scheduler', 'workflow', 'network', 'load-balancer', 'realtime-gateway', 'service', 'queue', 'cache', 'cdn', 'search-index', 'stream', 'topic', 'object-storage', 'database'])
