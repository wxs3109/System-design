...
import {
  ...
  globalRouterConfigSchema,
  loadBalancerConfigSchema,
  networkConfigSchema,
  objectStorageConfigSchema,
  queueConfigSchema,
  realtimeGatewayConfigSchema,
...
export const builtInComponentManifests = [
  ...
  {
    type: 'global-router', version: 1, label: 'Global Router', description: 'Steers cached client cohorts across explicit regional targets using geo, weighted, or health-aware routing.', category: 'gateway', iconToken: 'globe', color: '#be185d',
    configSchema: globalRouterConfigSchema, createDefaultConfig: () => ({ routingPolicy: 'geo', capacity: 10_000, lookupTimeMs: 1, jitterMs: 0.2, maxQueueSize: 100_000, decisionTtlMs: 60_000, healthCheckIntervalMs: 1_000, unhealthyThreshold: 2, healthyThreshold: 2, failoverDelayMs: 5_000 }),
    configFields: [
      { kind: 'select', key: 'routingPolicy', label: 'Routing policy', options: [{ value: 'geo', label: 'Geo affinity' }, { value: 'weighted', label: 'Weighted' }, { value: 'health-aware', label: 'Health aware' }] },
      { kind: 'number', key: 'capacity', label: 'Concurrent lookups', min: 1, step: 1 },
      { kind: 'number', key: 'lookupTimeMs', label: 'Lookup time (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'jitterMs', label: 'Lookup jitter (ms)', min: 0, step: 0.1 },
      { kind: 'number', key: 'maxQueueSize', label: 'Max queue', min: 0, step: 1 },
      { kind: 'number', key: 'decisionTtlMs', label: 'Decision TTL (ms)', min: 0.001, step: 1_000 },
      { kind: 'number', key: 'healthCheckIntervalMs', label: 'Health observation interval (ms)', min: 0.001, step: 100 },
      { kind: 'number', key: 'unhealthyThreshold', label: 'Failures before unhealthy', min: 1, step: 1 },
      { kind: 'number', key: 'healthyThreshold', label: 'Successes before recovery', min: 1, step: 1 },
      { kind: 'number', key: 'failoverDelayMs', label: 'Failover propagation (ms)', min: 0, step: 100 },
    ], ports: [requestInput, requestOutput], capabilities: ['global-routing', 'geo-routing', 'weighted-routing', 'health-aware-routing', 'decision-cache', 'failover-propagation'], emittedMetrics: ['latency', 'utilization', 'queue', 'routing-decisions', 'cache-hit-rate', 'requests-per-region', 'failovers', 'failover-delay'], supportedFaults: ['node-down', 'latency-spike', 'capacity-drop'], supportedNodePolicies: ['rate-limit'], runtimeBehavior: 'global-router-v1',
    describeConfig: (config) => `${config.routingPolicy} · TTL ${config.decisionTtlMs} ms · failover ${config.failoverDelayMs} ms`,
  },
  {
    type: 'realtime-gateway', version: 1, label: 'Realtime Gateway', description: 'Maintains long-lived channel memberships and broadcasts with per-connection backpressure.', category: 'gateway', iconToken: 'radio-tower', color: '#db2777',
    configSchema: realtimeGatewayConfigSchema, createDefaultConfig: () => ({ maxConnections: 100_000, connectionDurationMs: 60_000, maxChannelsPerConnection: 10, defaultChannelCount: 100, maxConcurrentMessages: 1_000, handshakeTimeMs: 2, broadcastBaseTimeMs: 1, fanOutTimePerConnectionMs: 0.01, defaultMessageBytes: 1_024, outboundBandwidthMbps: 10, slowConnectionFraction: 0, slowConnectionBandwidthMbps: 0.1, maxPendingBytesPerConnection: 1_048_576, overflowPolicy: 'drop-message', jitterMs: 0.5, errorRate: 0, maxQueueSize: 100_000 }),
    configFields: [
      { kind: 'number', key: 'maxConnections', label: 'Maximum connections', min: 1, step: 1 },
      { kind: 'number', key: 'connectionDurationMs', label: 'Connection lifetime (ms)', min: 0.001, step: 1_000 },
      { kind: 'number', key: 'maxChannelsPerConnection', label: 'Channels / connection', min: 1, step: 1 },
      { kind: 'number', key: 'defaultChannelCount', label: 'Default channel count', min: 1, step: 1 },
      { kind: 'number', key: 'maxConcurrentMessages', label: 'Concurrent messages', min: 1, step: 1 },
      { kind: 'number', key: 'handshakeTimeMs', label: 'Handshake time (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'broadcastBaseTimeMs', label: 'Broadcast base time (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'fanOutTimePerConnectionMs', label: 'Fan-out / connection (ms)', min: 0, step: 0.001 },
      { kind: 'number', key: 'defaultMessageBytes', label: 'Message bytes', min: 1, step: 128 },
      { kind: 'number', key: 'outboundBandwidthMbps', label: 'Outbound Mbps / connection', min: 0.001, step: 1 },
      { kind: 'number', key: 'slowConnectionFraction', label: 'Slow connection fraction', min: 0, max: 1, step: 0.05 },
      { kind: 'number', key: 'slowConnectionBandwidthMbps', label: 'Slow connection Mbps', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'maxPendingBytesPerConnection', label: 'Pending bytes / connection', min: 1, step: 1_024 },
      { kind: 'select', key: 'overflowPolicy', label: 'Overflow policy', options: [{ value: 'drop-message', label: 'Drop message' }, { value: 'disconnect', label: 'Disconnect client' }] },
      { kind: 'number', key: 'jitterMs', label: 'Jitter (ms)', min: 0, step: 0.1 },
      { kind: 'number', key: 'maxQueueSize', label: 'Max message queue', min: 0, step: 1 },
      { kind: 'number', key: 'errorRate', label: 'Error rate (0–1)', min: 0, max: 1, step: 0.001 },
    ], ports: [requestInput, requestOutput], capabilities: ['long-lived-connections', 'channel-membership', 'broadcast', 'connection-backpressure'], emittedMetrics: ['active-connections', 'rejected-connections', 'channel-memberships', 'broadcast-fan-out', 'pending-bytes', 'dropped-messages', 'slow-client-disconnects'], supportedFaults: ['node-down', 'latency-spike', 'capacity-drop'], supportedNodePolicies: ['rate-limit'], runtimeBehavior: 'realtime-gateway-v1',
    describeConfig: (config) => `${config.maxConnections} connections · ${config.defaultChannelCount} channels · ${config.overflowPolicy}`,
  },
  {
  ...
  {
    type: 'search-index', version: 1, label: 'Search Index', description: 'Indexes document mutations after refresh delay and fans queries across shard copies.', category: 'database', iconToken: 'search', color: '#7c3aed',
    configSchema: searchIndexConfigSchema, createDefaultConfig: () => ({ shardCount: 6, replicasPerShard: 1, maxConcurrentRequestsPerCopy: 100, maxQueueSize: 100_000, writeRatio: 0.2, keySpaceSize: 1_000_000, hotKeyProbability: 0, indexingDelayMs: 200, refreshIntervalMs: 1_000, replicaRefreshDelayMs: 100, queryBaseTimeMs: 2, shardQueryTimeMs: 4, fanOutTimePerShardMs: 0.2, mergeTimePerCandidateMs: 0.01, defaultResultLimit: 20, indexWriteTimeMs: 3, indexingThroughputMbps: 500, jitterMs: 1, errorRate: 0 }),
    configFields: [
      { kind: 'number', key: 'shardCount', label: 'Primary shards', min: 1, step: 1 },
      { kind: 'number', key: 'replicasPerShard', label: 'Replicas / shard', min: 0, step: 1 },
      { kind: 'number', key: 'maxConcurrentRequestsPerCopy', label: 'Concurrency / copy', min: 1, step: 1 },
      { kind: 'number', key: 'maxQueueSize', label: 'Max queue', min: 0, step: 1 },
      { kind: 'number', key: 'writeRatio', label: 'Index-write ratio (0–1)', min: 0, max: 1, step: 0.05 },
      { kind: 'number', key: 'keySpaceSize', label: 'Document key space', min: 1, step: 1 },
      { kind: 'number', key: 'hotKeyProbability', label: 'Hot-key probability (0–1)', min: 0, max: 1, step: 0.05 },
      { kind: 'number', key: 'indexingDelayMs', label: 'Indexing delay (ms)', min: 0, step: 10 },
      { kind: 'number', key: 'refreshIntervalMs', label: 'Refresh interval (ms)', min: 0.001, step: 100 },
      { kind: 'number', key: 'replicaRefreshDelayMs', label: 'Replica refresh delay (ms)', min: 0, step: 10 },
      { kind: 'number', key: 'queryBaseTimeMs', label: 'Query coordination (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'shardQueryTimeMs', label: 'Shard query time (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'fanOutTimePerShardMs', label: 'Fan-out / shard (ms)', min: 0, step: 0.1 },
      { kind: 'number', key: 'mergeTimePerCandidateMs', label: 'Merge / candidate (ms)', min: 0, step: 0.001 },
      { kind: 'number', key: 'defaultResultLimit', label: 'Query result limit', min: 1, step: 1 },
      { kind: 'number', key: 'indexWriteTimeMs', label: 'Index-write base (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'indexingThroughputMbps', label: 'Indexing throughput (Mbps)', min: 0.001, step: 10 },
      { kind: 'number', key: 'jitterMs', label: 'Jitter (ms)', min: 0, step: 0.1 },
      { kind: 'number', key: 'errorRate', label: 'Error rate (0–1)', min: 0, max: 1, step: 0.001 },
    ], ports: [requestInput, messageInput, requestOutput], capabilities: ['storage', 'document-indexing', 'near-real-time-refresh', 'shard-query-fan-out', 'replica-search', 'candidate-merge'], emittedMetrics: ['search-queries', 'stale-query-rate', 'indexing-backlog', 'visible-documents', 'shard-searches', 'candidates-merged'], supportedFaults: ['node-down', 'latency-spike', 'capacity-drop'], supportedNodePolicies: ['rate-limit', 'backpressure'], runtimeBehavior: 'search-index-v1',
    describeConfig: (config) => `${config.shardCount} shards · ${config.replicasPerShard} replicas / shard · ${config.refreshIntervalMs} ms refresh`,
  },
  {
  ...
  {
    type: 'topic', version: 1, label: 'Topic', description: 'Fans each publication into independently retained subscription backlogs.', category: 'messaging', iconToken: 'radio-tower', color: '#ea580c',
    configSchema: topicConfigSchema, createDefaultConfig: () => ({ subscriptionCount: 2, maxRetainedMessages: 1_000_000, retentionMs: 86_400_000, batchSize: 100, acknowledgement: 'explicit', publishCapacity: 1_000, publishTimeMs: 2, deliveryTimeMs: 10, jitterMs: 1, maxQueueSize: 1_000_000, errorRate: 0 }),
    configFields: [
      { kind: 'number', key: 'subscriptionCount', label: 'Subscriptions', min: 1, step: 1 },
      { kind: 'number', key: 'maxRetainedMessages', label: 'Max retained messages', min: 1, step: 1 },
      { kind: 'number', key: 'retentionMs', label: 'Retention (ms)', min: 0.001, step: 1_000 },
      { kind: 'number', key: 'batchSize', label: 'Delivery batch size', min: 1, step: 1 },
      { kind: 'select', key: 'acknowledgement', label: 'Acknowledgement', options: [{ value: 'auto', label: 'Auto' }, { value: 'explicit', label: 'Explicit' }] },
      { kind: 'number', key: 'publishCapacity', label: 'Publish capacity', min: 1, step: 1 },
      { kind: 'number', key: 'publishTimeMs', label: 'Publish time (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'deliveryTimeMs', label: 'Delivery time (ms)', min: 0.001, step: 0.1 },
      { kind: 'number', key: 'jitterMs', label: 'Jitter (ms)', min: 0, step: 0.1 },
      { kind: 'number', key: 'maxQueueSize', label: 'Max publish queue', min: 0, step: 1 },
      { kind: 'number', key: 'errorRate', label: 'Error rate (0–1)', min: 0, max: 1, step: 0.001 },
    ], ports: [messageInput, messageOutput], capabilities: ['buffering', 'publish-subscribe', 'independent-subscriptions', 'retention', 'per-subscription-acknowledgement', 'asynchronous-delivery'], emittedMetrics: ['publish-rate', 'fan-out-copies', 'subscription-backlog', 'acknowledged-deliveries', 'expired-deliveries'], supportedFaults: ['node-down', 'latency-spike', 'capacity-drop'], supportedNodePolicies: ['rate-limit', 'backpressure'], runtimeBehavior: 'topic-v1',
    describeConfig: (config) => `${config.subscriptionCount} subscriptions · ${config.retentionMs} ms retention · batch ${config.batchSize}`,
  },
  {
